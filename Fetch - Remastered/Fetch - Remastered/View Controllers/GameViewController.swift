...
var globalScene = GameScene(size: UIScreen.main.bounds.size)
var globalTestScene = testScene(size: UIScreen.main.bounds.size)
...
class GameViewController: UIViewController{
    ...
    func setup() {
        ...
        PresentScene()
        setupButtons(button: gameModifier, image: UIImage(named: "Profile")!, action: #selector(modifiersTapped), leading: globalScene.width(c: 280), trailing: globalScene.width(c: -12), height: globalScene.width(c: 122), y: frame.height / 2 - globalScene.height(c: 100))
        setupButtons(button: profile, image: UIImage(named: "Game Modifiers")!, action: #selector(profileTapped), leading: globalScene.width(c: 146), trailing: globalScene.width(c: -146), height: globalScene.width(c: 122), y: frame.height / 2 - globalScene.height(c: 100))
        setupButtons(button: Gold, image: UIImage(named: "Game Modifiers")!, action: #selector(GoldTapped), leading: globalScene.width(c: 12), trailing: globalScene.width(c: -280), height: globalScene.width(c: 122), y: frame.height / 2 - globalScene.height(c: 100))
...
//        print("THIS IS A SUBVIEW", view.subviews)
    ...
    @objc func GoldTapped() {
        if !globalScene.throwing && globalScene.tutorialComplete || !globalScene.throwing && globalScene.currentSlide.name == "shops"{
            present(GoldViewController(), animated: true, completion: nil)
            globalScene.playBeepSound()
        }
    }
